...
from server.data_processors import DataCleaner
...
class TestDataCleaner(TestCase):
        ...
        self.data_frame = pd.DataFrame(
            {
                ...
                "season_round": [
                    f"round {np.random.randint(1, 24)}" for _ in range(10)
                ],
            }
        )
    ...
    def test_transform(self):
        with self.subTest(data_frame=self.data_frame):
            ...
            self.assertTrue(
                all((col in data_frame.columns for col in ["round_number", "year"]))
            )
            self.assertFalse(
                any(
                    (
                        col in data_frame.columns
                        for col in ["datetime", "season_round", "venue", "crowd"]
                    )
                )
            )
