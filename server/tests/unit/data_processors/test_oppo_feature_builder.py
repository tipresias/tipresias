...
class TestOppoFeatureBuilder(TestCase):
    def setUp(self):
        self.match_cols = ['team', 'oppo_team', 'score', 'oppo_score', 'year',
                           'round_number']
    ...
    def test_transform(self):
        ...
        with self.subTest(data_frame=valid_data_frame,
            ...
            for match_col in match_cols:
                if match_col not in ['team', 'score']:
                    self.assertNotIn(f'oppo_{match_col}',
            ...
            self.assertEqual(
                len(np.setdiff1d(transformed_df['kicks'],
                                 transformed_df['oppo_kicks'])),
                0
            )
            self.assertEqual(
                len(np.setdiff1d(transformed_df['marks'],
                                 transformed_df['oppo_marks'])),
                0
            )
        ...
        with self.subTest(data_frame=valid_data_frame,
            ...
            oppo_feature_cols = self.oppo_feature_cols
            transformed_df = (self
                              .builder(oppo_feature_cols=oppo_feature_cols)
                              .transform(data_frame))
            ...
            self.assertEqual(
                len(np.setdiff1d(
                    transformed_df['marks'],
                    transformed_df['oppo_marks']
                )),
                0
            )

        with self.subTest(match_cols=self.match_cols, oppo_feature_cols=self.oppo_feature_cols):
            with self.assertRaises(ValueError):
                (self
                 .builder(match_cols=self.match_cols,
                          oppo_feature_cols=self.oppo_feature_cols)
