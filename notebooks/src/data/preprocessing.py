...
def betting_df():
    return (pd.concat([raw_match_df(), raw_betting_df()], axis=1)
...
def betting_model_df(test_year='2017'):
    ...
    return (df[df['datetime'] < f'{test_year}-01-01']
                  .assign(round_number=df['season_round'].map(get_round_number),
...
def team_betting_model_df(df):
    return pd.concat([team_df(df, team_type='home'), team_df(df, team_type='away')], join='inner').sort_index()
...
def team_year_ladder_position(df):
    ...
    ladder_multi_index = (pd.MultiIndex
                            .from_tuples(ladder_index,
