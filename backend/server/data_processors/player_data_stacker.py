...
REQUIRED_COLS: List[str] = [
    'playing_for', 'home_team', 'away_team', 'home_score', 'away_score',
    'match_id'
]
...
class PlayerDataStacker():
    ...
    def transform(self, data_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        return (pd
                .concat(team_dfs, sort=True)
                .drop(['match_id', 'playing_for'], axis=1))

    def __team_df(self, data_frame: pd.DataFrame, team_type: str) -> pd.DataFrame:
        return self.__sort_columns(
            data_frame
            [data_frame['playing_for'] == data_frame[f'{team_type}_team']]
