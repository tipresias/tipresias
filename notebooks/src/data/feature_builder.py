...
class FeatureBuilder():
    ...
    def add_last_week_score(self):
        self.df.loc[:, 'last_week_score'] = self.df.groupby(level=TEAM_LEVEL)['score'].shift()
        self.df.loc[:, 'last_week_oppo_score'] = self.df.groupby(level=TEAM_LEVEL)['oppo_score'].shift()
    ...
    def add_rolling_features(self, oppo_col=True):
        ...
        self.df.loc[:, 'rolling_last_week_win_rate'] = self.__rolling_last_week_win_rate_col()
    ...
    def __cum_percent_col(self):
        ...
        return self.__cum_col(self.df['last_week_score']) / self.__cum_col(self.df['last_week_oppo_score'])
    ...
    def __ladder_position_col(self):
        ...
        ladder_pivot_table = (self.df[['cum_win_points', 'cum_percent']]
        ...
        ladder_multi_index = pd.MultiIndex.from_tuples(ladder_index,
                                                       names=('team', 'year', 'round_number'))
    ...
    def __win_streak_col(self):
        last_week_win_groups = self._last_week_results.groupby(level=TEAM_LEVEL, group_keys=False)
        streak_groups = []
        ...
        for team_group_key, team_group in last_week_win_groups:
            ...
            for idx, result in enumerate(team_group):
                ...
                # 0 represents loss
                    ...
                    raise ValueError(f'No results should be negative, but {result} is at index {idx} of group {team_group_key}')
                else:
                    streaks.append(0)
    ...
    def __cum_col(self, series):
        return (series.groupby(level=[TEAM_LEVEL, YEAR_LEVEL])
                      .cumsum()
    ...
    def __add_oppo_col(self, col_name):
        ...
        oppo_col = (self.df.loc[:, ['year', 'round_number', 'oppo_team', col_name]]
