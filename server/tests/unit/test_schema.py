...
class TestSchema(TestCase):
    def setUp(self):
        ...
        (TeamMatch(team=home_team, match=new_match, at_home=True, score=150)
         .save())
        (TeamMatch(team=away_team, match=new_match, at_home=False, score=100)
         .save())
        (TeamMatch(team=home_team, match=old_match, at_home=True, score=150)
         .save())
        (TeamMatch(team=away_team, match=old_match, at_home=False, score=100)
         .save())
    ...
    def assert_correct_results(self, results, expected_results):
        ...
        for idx, result in enumerate(results):
            ...
            self.assertEqual(
                dict(result['match']), expected_result['match'])
            self.assertEqual(
                dict(result['mlModel']), expected_result['mlModel'])
