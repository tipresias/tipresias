...
class DataBuilder():
    def __init__(self, data_classes, csv_paths, shared_index_cols=['date', 'venue', 'home_team', 'away_team']):
        if len(csv_paths) != len(data_classes):
            raise ValueError('csv_paths and data_classes arguments must be same length')

        abs_paths = [os.path.join(project_path, csv_path) for csv_path in csv_paths]

        self.dfs = [data_class(abs_paths[idx], shared_index_cols).data_frame()
...
class BettingData():
    ...
    def data_frame(self):
            home_df, away_df = self.__split_home_away('home'), self.__split_home_away('away')

            return (home_df.merge(away_df, on=self.index_col)
                           .reset_index()
                           .set_index(self.shared_index_cols))

    def __split_home_away(self, team_type):
        return (self._data[self._data['home'] == int(team_type == 'home')]
                   .drop('home', axis=1)
...
class MatchData():
    ...
    def data_frame(self):
        return (self._data.rename(columns={'date': 'datetime'})
