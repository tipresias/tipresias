...
UnzippedGroups = Tuple[Sequence[Sequence[TeamMatch]],
...
class Command(BaseCommand):
    ...
    def handle(self, *_args, **_kwargs) -> None:  # pylint disable=W0613
        ...
        filtered_matches = (match_results
                            .loc[match_results['season'] >= 2011, COLUMNS]
                            .assign(date=self.__convert_to_datetime))
        prediction_data_frame: pd.DataFrame = pd.read_csv(
            f'{PROJECT_PATH}/data/model_predictions.csv'
        )
        ...
        team_match_groups, prediction_groups = zip(*[  # pylint disable=C0301
    ...
    def __build_team_matches_and_predictions(
        self,
        get_match_predictions: Callable[[Record], pd.DataFrame],
        record: Record
        ...
        build_prediction = partial(
            self.__build_prediction, match, home_team, away_team
        )

        team_matches = self.__build_team_matches(
            match, home_team, away_team, record
        )
    ...
    def __build_prediction(self,
                           ...
                           home_team: Team,
                           away_team: Team,
                           prediction_datum: Record) -> Prediction:
    ...
    def __seed_teams(self, match_results: pd.DataFrame) -> None:
        ...
        teams: List[Team] = [
            self.__build_team(team_name) for team_name in team_names
        ]
    ...
    def __convert_to_datetime(data_frame: pd.DataFrame) -> pd.DataFrame:
        return (pd
                ...
                .to_datetime(data_frame['date'], unit='D')
                .dt
                .to_pydatetime())
    ...
    @staticmethod
    def __get_match_predictions(data_frame: pd.DataFrame,
    ...
    @staticmethod
    def __winning_team(home_team: Team,
                       away_team: Team,
                       prediction_datum: Record) -> Team:
        ...
        if predicted_home_win:
            if prediction_datum['home_team'] != home_team.name:
                raise ValueError('Prediction home team name '
    ...
    def __build_team_matches(match: Match,
                             home_team: Team,
                             away_team: Team,
                             record: Record) -> Tuple[TeamMatch, TeamMatch]:
        home_team_match = TeamMatch(
            team=home_team,
            match=match,
            at_home=True,
            score=record['home_points']
        )
        away_team_match = TeamMatch(
            team=away_team,
            match=match,
            at_home=False,
            score=record['away_points']
        )
