...
class DataTransformer():
    ...
    def clean(self,
              min_year='01',
              max_year='2016',
              drop_cols=['venue', 'crowd']):
        return (self.df[(self.df['datetime'] >= f'{min_year}-01-01') & (self.df['datetime'] <= f'{max_year}-12-31')]
                       .assign(round_number=self.__extract_round_number,
                               year=self.__extract_year)
                       .drop(drop_cols + ['datetime', 'season_round'], axis=1))
    ...
    def __team_df(self, team_type, **kwargs):
        ...
        return (df.rename(columns=self.__replace_col_names(team_type, oppo_team_type))
