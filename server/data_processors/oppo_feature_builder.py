...
class OppoFeatureBuilder():
    ...
    def __init__(self, match_cols: List[str] = [], oppo_feature_cols: List[str] = []) -> None:
        if any(match_cols) and any(oppo_feature_cols):
            raise ValueError("To avoid conflicts, you can't include both match_cols "
    ...
    def transform(self, data_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.concat(
            [transform_data_frame,
             self.__oppo_features(transform_data_frame)],
            axis=1
        )
    ...
    def __oppo_features(self, data_frame: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        oppo_cols = {
            col_name: f'oppo_{col_name}' for col_name in cols_to_convert
        }
