...
def measure_regressor(estimator, data, cv=5):
    ...
    return (cross_val_score(estimator, X_train, y_train, scoring=make_scorer(regression_accuracy), cv=cv),
            cross_val_score(estimator, X_train, y_train, scoring='neg_mean_absolute_error', cv=cv) * -1,
...
def measure_classifier(estimator, data, cv=5):
    ...
    try:
        cv_error_score = cross_val_score(estimator, X_train, y_train, scoring='neg_log_loss', cv=cv)
...
def measure_estimators(estimators, data, model_type='regression', cv=5):
    ...
    for estimator in estimators:
        ...
        if model_type == 'regression':
            cv_accuracies, cv_errors, test_accuracy, test_error = measure_regressor(pipeline, data, cv=cv)
        else:
            cv_accuracies, cv_errors, test_accuracy, test_error = measure_classifier(pipeline, data, cv=cv)
