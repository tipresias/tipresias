...
def tipresias_all_data_predictions() -> pd.DataFrame:
    ...
    data = AllXGBData(train_years=(None, None), test_years=(None, None))
    estimator = AllXGB()
...
def main():
    pd.concat(
        [
            ...
            tipresias_all_data_predictions(),
        ]
    ).to_csv(f"{BASE_DIR}/data/model_predictions.csv", index=False)
